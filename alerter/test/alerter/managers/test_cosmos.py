...
from src.utils.constants.rabbitmq import (
    ...
    COSMOS_NODE_ALERT_ROUTING_KEY, COSMOS_ALERTERS_MAN_CONFIGS_QUEUE_NAME,
    COSMOS_ALERTERS_MAN_HB_QUEUE_NAME, COSMOS_NETWORK_ALERT_ROUTING_KEY,
    PING_ROUTING_KEY, COSMOS_ALERTS_CONFIGS_ROUTING_KEY,
...
class TestCosmosAlertersManager(unittest.TestCase):
    def setUp(self) -> None:
        # Some dummy objects
        self.dummy_logger = logging.getLogger('Dummy')
        self.dummy_logger.disabled = True
        self.manager_name = 'test_cosmos_alerters_manager'
        ...
        self.routing_key = 'chains.cosmos.regen.alerts_config'
        self.parent_id = "chain_name_d21d780d-92cb-42de-a7c1-11b751654510"
        self.chain_name = 'cosmos regen'

        # Construct received configurations
        cosmos_config_metrics = [
        ...
        self.node_config_expected = {
            self.chain_name: CosmosNodeAlertsConfig(
                parent_id=self.parent_id,
                cannot_access_validator=filtered_received_configs[
                    'cannot_access_validator'],
                cannot_access_node=filtered_received_configs[
                    'cannot_access_node'],
                validator_not_active_in_session=filtered_received_configs[
                    'validator_not_active_in_session'],
                no_change_in_block_height_validator=filtered_received_configs[
                    'no_change_in_block_height_validator'],
                no_change_in_block_height_node=filtered_received_configs[
                    'no_change_in_block_height_node'],
                block_height_difference=filtered_received_configs[
                    'block_height_difference'],
                cannot_access_prometheus_validator=filtered_received_configs[
                    'cannot_access_prometheus_validator'],
                cannot_access_prometheus_node=filtered_received_configs[
                    'cannot_access_prometheus_node'],
                cannot_access_cosmos_rest_validator=filtered_received_configs[
                    'cannot_access_cosmos_rest_validator'],
        ...
        self.network_config_expected = {
            self.chain_name: CosmosNetworkAlertsConfig(
                parent_id=self.parent_id,
                new_proposal=filtered_received_configs['new_proposal'],
                proposal_concluded=filtered_received_configs[
                    'proposal_concluded'],
            )
        }

        self.test_manager = CosmosAlertersManager(
            self.dummy_logger, self.manager_name, self.rabbitmq)
        self.alerter_process_dict_example = {
            COSMOS_NODE_ALERTER_NAME: self.dummy_process_1,
            COSMOS_NETWORK_ALERTER_NAME: self.dummy_process_2
        }
        self.node_alerts_config_factory = CosmosNodeAlertsConfigsFactory()
        self.network_alerts_config_factory = CosmosNetworkAlertsConfigsFactory()
    ...
    def test_initialise_rabbitmq_initialises_everything_as_expected(
        ...
        self.test_manager.rabbitmq.basic_publish_confirm(
            ...
            routing_key=COSMOS_ALERTS_CONFIGS_ROUTING_KEY,
        ...
        expected_calls = [
            call(COSMOS_ALERTERS_MAN_HB_QUEUE_NAME,
                 self.test_manager._process_ping, True, False, None),
            call(COSMOS_ALERTERS_MAN_CONFIGS_QUEUE_NAME,
    ...
    @parameterized.expand([
        ([True, True], [],),
        ([False, True], [COSMOS_NODE_ALERTER_NAME],),
        ([True, False], [COSMOS_NETWORK_ALERTER_NAME],),
        ([False, False], [COSMOS_NODE_ALERTER_NAME,
                          COSMOS_NETWORK_ALERTER_NAME],),
    ])
    @freeze_time("2012-01-01")
    @mock.patch.object(multiprocessing.Process, "join")
    @mock.patch.object(multiprocessing.Process, "is_alive")
    @mock.patch.object(CosmosAlertersManager,
                       "_create_and_start_alerter_processes")
    @mock.patch.object(CosmosAlertersManager, "_send_heartbeat")
    ...
    def test_create_and_start_alerter_processes_if_processes_not_running(
        ...
        self.assertEqual(start_cosmos_node_alerter,
        ...
        self.assertEqual(start_cosmos_network_alerter,
        ...
        expected_alert_1 = ComponentResetAlert(
            COSMOS_NODE_ALERTER_NAME, datetime.now().timestamp(),
            CosmosNodeAlerter.__name__)
        expected_alert_2 = ComponentResetAlert(
            COSMOS_NETWORK_ALERTER_NAME, datetime.now().timestamp(),
            CosmosNetworkAlerter.__name__)
        expected_calls = [
            call(expected_alert_1.alert_data, COSMOS_NODE_ALERT_ROUTING_KEY),
            call(expected_alert_2.alert_data, COSMOS_NETWORK_ALERT_ROUTING_KEY),
        ]
    ...
    def test_process_configs_if_non_empty_configs_received(
        ...
        expected_alert_1 = ComponentResetAlert(
            COSMOS_NODE_ALERTER_NAME, datetime.now().timestamp(),
            CosmosNodeAlerter.__name__, self.parent_id, self.chain_name
        )
        expected_alert_2 = ComponentResetAlert(
            COSMOS_NETWORK_ALERTER_NAME, datetime.now().timestamp(),
            CosmosNetworkAlerter.__name__, self.parent_id, self.chain_name
        )

        self.assertEqual(self.node_config_expected,
    ...
    @mock.patch.object(CosmosAlertersManager, "_send_data")
