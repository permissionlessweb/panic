...
class CosmosRestServerApiWrapper(ApiWrapper):
    ...
    def get_syncing(self, cosmos_rest_url: str) -> Dict:
        """
        This function retrieves data from the cosmos_rest_url/cosmos/base/tendermint/v1beta1/syncing endpoint,
        ...
        :return: Retrieves data from the cosmos_rest_url/cosmos/base/tendermint/v1beta1/syncing endpoint
        """
    ...
    def get_staking_validators(
        ...
        endpoints, depending on the inputted function parameters.
        :param cosmos_rest_url: The Cosmos REST url of the data source
...
class CometbftRpcApiWrapper(ApiWrapper):
    """
    This API acts as a wrapper around the Cometbft RPC interface. The
    functions implemented in this API wrapper are currently implemented for all
    Cometbft versions which were considered during development, however, for
    ...
    received.
    Docs: https://docs.cometbft.com/v1.0/rpc/
    Considered Cometbft Versions: v0.38.*   

    NOTE: When developing new function parameters always put 'params' as the
        : last argument if it is needed. This is a requirement if you want to
        : use CosmosMonitor._get_cometbft_data_with_count
    """
    ...
    def get_block(self, cometbft_rpc_url: str, params: Dict = None) -> Dict:
        """
        This function retrieves data from the
        cometbft_rpc_url/block?height=<height> endpoint if height is included
        in the params Dict. Otherwise, it retrieves data from the
        cometbft_rpc_url/block endpoint
        :param cometbft_rpc_url: The cometbft RPC url of the data source
        :param params: Parameters that need to be added to the endpoint
        :return: Data from cometbft_rpc_url/block?height=<height> if height is
                 included in the params Dict. Otherwise it retrieves data from
                 the cometbft_rpc_url/block endpoint
        """
        endpoint = cometbft_rpc_url + '/block'
        return get_cosmos_json(endpoint=endpoint, logger=self.logger,
                               verify=self.verify, timeout=self.timeout,
                               params=params)

    def get_block_results(self, cometbft_rpc_url: str,
                          params: Dict = None) -> Dict:
        """
        This function retrieves data from the
        cometbft_rpc_url/block_results?height=<height> endpoint if height is
        included in the params Dict. Otherwise, it retrieves data from the
        cometbft_rpc_url/block_results endpoint
        :param cometbft_rpc_url: The cometbft RPC url of the data source
        :param params: Parameters that need to be added to the endpoint
        :return: Data from cometbft_rpc_url/block_results?height=<height> if
                 height is included in the params Dict. Otherwise it retrieves
                 data from the cometbft_rpc_url/block_results endpoint
        """
        endpoint = cometbft_rpc_url + '/block_results'
        return get_cosmos_json(endpoint=endpoint, logger=self.logger,
                               verify=self.verify, timeout=self.timeout,
                               params=params)

    def get_status(self, cometbft_rpc_url: str) -> Dict:
        """
        This function retrieves data from the cometbft_rpc_url/status endpoint
        :param cometbft_rpc_url: The cometbft RPC url of the data source
        :return: Data from cometbft_rpc_url/status
        """
        endpoint = cometbft_rpc_url + '/status'
        return get_cosmos_json(endpoint=endpoint, logger=self.logger,
                               verify=self.verify, timeout=self.timeout)

    def get_validators(self, cometbft_rpc_url: str,
                       params: Dict = None) -> Dict:
        """
        This function retrieves data from the cometbft_rpc_url/validators?
        ...
        Otherwise, it retrieves data from the cometbft_rpc_url/validators
        endpoint
        :param cometbft_rpc_url: The cometbft RPC url of the data source
        :param params: Parameters that need to be added to the endpoint
        :return: Data from cometbft_rpc_url/validators?
                 ...
                 cometbft_rpc_url/validators endpoint
        """
        endpoint = cometbft_rpc_url + '/validators'
        return get_cosmos_json(endpoint=endpoint, logger=self.logger,
                               verify=self.verify, timeout=self.timeout,
                               params=params)
    ...
    def execute_with_checks(self, function, args: List[Any],
        ...
        CometbftRPCIncompatibleException as this means that the endpoint
        cannot be found at the node's Cometbft RPC interface.
        Apart from version compatibility we will also check if an error is
        returned by the API call. In that case a CometbftRPCCallException will
        be raised.
        :param function: The wrapper call to execute
        :param args: The arguments to be passed to the API call
        :param node_name: The name of the data source.
        :return: The function return if successful
               : CometbftRPCIncompatibleException if the API call is not
                 supported by the node's Cometbft RPC version.
               : CometbftRPCCallException if an error is returned by the API
                 Call
        """
        ret = function(*args)
        ...
        if (isinstance(ret, Dict)
            ...
            self.logger.error('Function {} failed. Error: {}'.format(
                function, error_msg))
            raise CometbftRPCCallException(function, error_msg)
        elif type(ret) == Response and ret.status_code == 404:
            self.logger.error('The Cometbft RPC version of node {} does not '
                              'support function {}.'.format(node_name,
                                                            function))
            raise CometbftRPCIncompatibleException(node_name)
